...
BlockLayout::BlockLayout(std::vector<uint32_t> attr_sizes)
  ...
  // for (uint32_t size UNUSED_ATTRIBUTE : attr_sizes_)
...
uint32_t BlockLayout::ComputeTupleSize() const {
  ...
  for (auto size : attr_sizes_) result += static_cast<uint32_t>(INT32_MAX & size);
  return result;
}
